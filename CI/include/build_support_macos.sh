...
_build_checks() {
    ...
    CI_PRODUCT_VERSION=$(/bin/cat "${CI_WORKFLOW}" | /usr/bin/sed -E '/macos-.+-build/,/defaults:/!d' | /usr/bin/sed -En "s/[ ]+${PRODUCT_NAME_U}_VERSION: '(.+)'/\1/p")
    CI_PRODUCT_HASH=$(/bin/cat "${CI_WORKFLOW}" | /usr/bin/sed -E '/macos-.+-build/,/defaults:/!d' | /usr/bin/sed -En "s/[ ]+${PRODUCT_NAME_U}_HASH: '([0-9a-f]+)'/\1/p")
